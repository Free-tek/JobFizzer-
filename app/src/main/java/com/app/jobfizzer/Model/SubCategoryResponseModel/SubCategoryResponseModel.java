package com.app.jobfizzer.Model.SubCategoryResponseModel;

import com.google.gson.annotations.Expose;
import com.google.gson.annotations.SerializedName;

import java.io.Serializable;
import java.util.List;

public class SubCategoryResponseModel implements Serializable {

    @Expose
    @SerializedName("list_subcategory")
    private List<List_subcategory> list_subcategory;
    @Expose
    @SerializedName("error_message")
    private String error_message;
    @Expose
    @SerializedName("error")
    private String error;

    public List<List_subcategory> getList_subcategory() {
        return list_subcategory;
    }

    public void setList_subcategory(List<List_subcategory> list_subcategory) {
        this.list_subcategory = list_subcategory;
    }

    public String getError_message() {
        return error_message;
    }

    public void setError_message(String error_message) {
        this.error_message = error_message;
    }

    public String getError() {
        return error;
    }

    public void setError(String error) {
        this.error = error;
    }
}
